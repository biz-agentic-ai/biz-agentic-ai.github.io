...
class SchemaEnforcer:
    def __init__(self, glossary_terms: List[GlossaryTerm]):
        self.term_index = self._build_index(glossary_terms)
        self.synonym_map = self._build_synonym_map(glossary_terms)

    def validate_triple(self, triple: Triple) -> ValidationResult:
        """추출된 트리플의 스키마 적합성 검증"""

        # 1. Exact Match 검사
        if self._exact_match(triple.subject):
            subject_status = "ACCEPT"
            subject_uri = self.term_index[triple.subject].uri

        # 2. Synonym Match 검사
        elif self._synonym_match(triple.subject):
            subject_status = "REMAP"
            canonical = self.synonym_map[triple.subject]
            subject_uri = self.term_index[canonical].uri

        # 3. Fuzzy Match 검사 (임계값 0.85)
        elif self._fuzzy_match(triple.subject) > 0.85:
            subject_status = "REVIEW"  # 관리자 검토 큐로 전송
            subject_uri = None

        # 4. 불일치
        else:
            subject_status = "REJECT"
            subject_uri = None
        ...
        return ValidationResult(
            ...
            predicate_valid=self._validate_predicate(triple.predicate),
            action=self._determine_action(subject_status)
        )

    def _determine_action(self, status: str) -> str:
        """검증 결과에 따른 처리 액션 결정"""
        action_map = {
            "ACCEPT": "STORE",      # 지식 그래프에 저장
            "REMAP": "STORE",       # 정규화 후 저장
            "REVIEW": "QUEUE",      # 관리자 검토 큐로 전송
            "REJECT": "DISCARD"     # 폐기 및 로깅
        }
        return action_map.get(status, "DISCARD")
```
...
```python
class CQValidator:
    def __init__(self, ontology: Ontology, llm: LLM):
        self.ontology = ontology
        self.llm = llm

    def validate_cq(self, cq: CompetencyQuestion) -> CQValidationResult:
        """CQ가 현재 온톨로지로 답변 가능한지 검증"""

        # 1. 개념 존재 검사
        concept_coverage = self._check_concept_coverage(cq.required_concepts)

        # 2. 관계 표현력 검사
        relationship_coverage = self._check_relationship_coverage(cq.required_relationships)

        # 3. 쿼리 생성 시뮬레이션
        generated_query = self._simulate_query_generation(cq.question_natural)

        # 4. 쿼리 실행 가능성 검사
        query_executable = self._validate_query_syntax(generated_query)

        # 5. LLM-as-a-Judge 평가
        llm_evaluation = self._llm_evaluate(
            cq=cq,
            generated_query=generated_query,
            expected_pattern=cq.expected_query_pattern
        )

        return CQValidationResult(
            cq_id=cq.cq_id,
            ...
            missing_elements=self._identify_gaps(cq),
            recommendation=self._generate_recommendation(cq)
        )

    def _simulate_query_generation(self, question: str) -> str:
        """온톨로지 컨텍스트를 주입하여 쿼리 생성 시뮬레이션"""
        prompt = f"""
        Given the following ontology context:
        {self.ontology.to_documentation}
...
class ImpactAnalyzer:
    def __init__(self, graph_db: DozerDB, vector_db: Qdrant):
        self.graph = graph_db
        self.vector = vector_db

    def analyze_change_impact(self, change_event: ChangeEvent) -> ImpactReport:
        """변경 사항의 영향 범위 분석"""
        ...
        # 1. 직접 영향 노드 (1-hop)
        direct_neighbors = self._get_neighbors(
            change_event.entity_urn,
            hops=1
        )
        affected_nodes.update(direct_neighbors)

        # 2. 간접 영향 노드 (2-hop, 계층 구조 변경 시)
        if change_event.change_type == "HIERARCHY_CHANGE":
            indirect_neighbors = self._get_neighbors(
                change_event.entity_urn,
                hops=2
            )
            affected_nodes.update(indirect_neighbors)

            # 서브트리 전체 영향
            subtree = self._get_subtree(change_event.entity_urn)
            affected_nodes.update(subtree)

        # 3. 영향받는 문서 청크 식별
        for node in affected_nodes:
            chunks = self._find_referencing_chunks(node)
            affected_documents.update(chunks)

        # 4. 영향 점수 계산
        impact_score = self._calculate_impact_score(
            num_nodes=len(affected_nodes),
            num_documents=len(affected_documents),
        ...
        return ImpactReport(
            ...
            recommended_strategy=self._recommend_strategy(impact_score),
            estimated_time=self._estimate_reindex_time(affected_documents)
        )

    def _recommend_strategy(self, impact_score: float) -> str:
        """영향 점수에 따른 업데이트 전략 추천"""
        if impact_score < 0.1:
            return "INCREMENTAL"  # 증분 업데이트
        elif impact_score < 0.5:
            return "PARTIAL_REBUILD"  # 부분 재구축
        else:
            return "FULL_REBUILD"  # 전체 재구축 (드문 경우)
```
...
class OntologyVersionManager:
    def rollback_to_version(self, target_version: str):
        ...
        for change in diff.changes:
            if change.type == "TERM_DELETED":
                self._restore_term(change.entity_urn, snapshot)
            elif change.type == "TERM_MODIFIED":
                self._revert_term(change.entity_urn, snapshot)

        # 4. RAG Store 동기화
        self._sync_rag_store(affected_terms=diff.affected_entities)

        # 5. 버전 메타데이터 업데이트
        self._update_version_metadata(
            new_version=self._increment_patch(target_version),
...
<!-- TODO(개발): Cypher 템플릿 20+ 상세 목록 추가 필요 - 현재 4개 핵심 패턴만 나열됨. PRD_Appendix_CDE.md §D.2, §E.1 참조 (초기 20개 템플릿 구현 및 테스트 필요) -->

#### 4.5.2.4 LLM Fallback 정책
...
| 지표명 | 정의 | 목표 기준 |
| :--- | :--- | :--- |
| **Routing Accuracy** | 올바른 경로로 라우팅된 질의 비율 | 95% 이상 (Phase 2: 97% 이상; 정식 기준은 §4.8 SSOT 참조) |
...
### 4.7 SKOS-DataHub 매핑 갭 해소 (보강: 리뷰 보고서 §2-4)
...
### 4.8 품질 지표 통합 정의 (보강: 리뷰 보고서 §2-3)

> **⚠️ 품질 지표 이중 정의 문제 (리뷰 보고서 §2-3)**

PRD 내에서 동일 지표가 다른 섹션에서 다른 기준으로 정의되는 문제가 있습니다. 아래를 **단일 진실 공급원(Single Source of Truth)**으로 정의합니다.

| 지표 | 통합 정의 | MVP 기준 | Phase 2 기준 | 참조 섹션 |
|------|----------|---------|-------------|----------|
| **Routing Accuracy** | Query Router의 올바른 에이전트/경로 선택 비율 | ≥ 0.95 | ≥ 0.97 | §3.4, §5.1 |
| **Hallucination Rate** | 근거 없는 정보가 응답에 포함된 비율 | ≤ 0.05 | ≤ 0.03 | §5.1, §5.3 |
| **Schema Compliance** | 스키마 강제성 통과 비율 (ACCEPT+REMAP) | ≥ 0.90 | ≥ 0.95 | §4.4.2.3, §5.1 |
| **CQ Pass Rate** | 적합성 질문 검증 통과율 | ≥ 0.80 | ≥ 0.95 | §4.4.3, §5.1 |
| **EX (Execution Accuracy)** | NL2SQL 실행 결과값 일치율 | ≥ 0.80 | ≥ 0.90 | §5.1, §5.3.4 |
| **ConflictResolutionScore** | 다중 소스 충돌 해결 품질 | ≥ 0.95 | ≥ 0.97 | §3.5.3, §5.1 |
| **Reasoning Accuracy** | 관계 기반 추론의 정확도 (Multi-hop 포함) | ≥ 0.85 | ≥ 0.90 | §4.4.1.3 |
