...
│   ├── rag-search.md
│   ├── agent-routing.md
│   └── analysis-workflow.md  ← alive-analysis ALIVE 루프 도메인 규칙
...
.analysis/                    ← alive-analysis 분석 파일 저장소 (Git 추적)
├── investigations/           ← Investigation 유형 분석 ("왜?")
├── models/                   ← Modeling 유형 분석 ("예측?")
├── simulations/              ← Simulation 유형 분석 ("만약?")
...
- Analysis Agent: alive-analysis ALIVE 루프에서 결론이 아닌 구조화된 질문만 생성 (분석적 판단은 사람이 내림)
...
#### Prompt Cache Optimization (프롬프트 캐시 최적화)

> **상세 전략:** Implementation Strategy §22, PRD_04c §4.3.10.10.3 참조

SEOCHO API 호출 시 프롬프트 캐싱 효율을 최대화하기 위한 규칙입니다. 프롬프트 캐싱은 프리픽스 매칭 방식이므로, 프리픽스 변경이 곧 캐시 무효화입니다.

**프롬프트 배치 순서 (Static First, Dynamic Last):**
1. Static System Prompt + Tool Definitions → 전역 캐시
2. Ontology Context + DDL Schema → 테넌트별 캐시
3. Session Context (Graphiti Facts) → 세션별 캐시
4. Conversation Messages → 매 턴 변경

**세션 중 금지 사항:**
- 도구 추가/제거 금지 → `defer_loading` 스텁 + `ToolSearch` 사용
- 모델 전환 금지 → 서브에이전트 핸드오프 사용
- 시스템 프롬프트 수정 금지 → `<system-reminder>` 메시지로 대체
- 도구 정의 순서 변경 금지 → `OrderedDict` 또는 정렬 고정
- 캐시 히트율 ≥ 70% 유지 (PRD_05 §5.1.3), 60% 미만 시 SEV-2 선언
...
|--------|-----------|----------------|
| Query Router Accuracy | ≥ 0.95 | ≥ 0.97 |
...
## Training 데이터 관리 안티패턴 (절대 하지 말 것)
- ❌ 사용자 피드백 없이 모든 생성 SQL을 자동 학습 → 👍 확인 + Schema Validation 통과 시에만 학습 (§3.10.2)
- ❌ Training YAML에 검증 안 된 SQL 추가 → CI/CD sqlfluff lint + 실행 테스트 통과 필수 (§4.2.4)
- ❌ 단일 YAML 파일에 전체 training 데이터 집중 → 도메인별 분할 (financial.yaml, customer.yaml 등)
- ❌ 코사인 유사도 > 0.9인 중복 Q-SQL 쌍 학습 → 임베딩 비교로 중복 제거 후 학습
- ❌ training YAML에 glossary_terms 태그 누락 → 온톨로지 필터링 품질 저하, 70% 이상 태깅 필수
- ❌ 수정 SQL을 DBA 검토 없이 바로 학습 → 사용자 수정 SQL은 반드시 DBA 승인 후 반영 (§3.10.3)
...
- ❌ 세션 중간에 도구 정의 추가/제거 → 프리픽스 캐시 전체 무효화, defer_loading 스텁 패턴 사용 (Implementation Strategy §22.3.1)
- ❌ 비용 절감 위해 간단한 질문에 모델 전환 (Opus→Haiku) → 캐시 모델별 고유, 서브에이전트 핸드오프로 대체 (§22.3.2)
- ❌ 시스템 프롬프트에 상세 타임스탬프 삽입 → 매 요청 캐시 미스 유발, system-reminder 메시지로 주입 (§22.5)
- ❌ 도구 정의 순서를 Dict/Set 순회로 비결정적 생성 → OrderedDict 또는 정렬된 리스트로 순서 고정 (§22.2)
- ❌ 컴팩션 시 별도 시스템 프롬프트/도구 세트 사용 → 부모 대화와 동일 프리픽스 사용 필수 (§22.4)
...
- 분석 유형 분류: Investigation ("왜?") / Modeling ("예측?") / Simulation ("만약?")
- 모드 선택: Quick (단일 MD, 비분석가/탐색) / Full (5개 버전 관리 MD, 분석가/보고용)
- 핵심 원칙: AI는 구조화된 질문만 생성, 분석 결론은 사람이 내림
- 산출물 위치: .analysis/{investigations,models,simulations,experiments,monitoring}/
- ID 체계: Quick=Q-YYYY-MMDD-NNN, Full=F-YYYY-MMDD-NNN
- SKILL.md 로딩: /analysis-init 실행 시에만 활성화 (비분석 세션은 미로딩)

## alive-analysis ↔ DataNexus 연계
- LOOK 단계: Vanna NL2SQL로 데이터 품질 자동 점검, DataHub Glossary Term 확인
- INVESTIGATE 단계: ApeRAG GraphRAG + DozerDB 그래프 추론
- VOICE 단계: 5개 페르소나별 응답 최적화 (PRD_07 §11.12.3)
- VOICE → Shaper: 반복 조회 분석을 Dashboard Promotion으로 승격 (PRD_02 §3.9)
- EVOLVE 단계: 분석 인사이트를 Graphiti 시간축 메모리에 축적 (Phase 3)

## 실험/모니터링 연계
- 실험 모듈: Ablation Study M1-M4, A1-A6에 사전등록/SRM 검사 적용 (PRD_05 §5.4)
- 4단계 메트릭 분류 (PRD_05 §5.1.5):
  - North Star: EX (Execution Accuracy)
  - Leading: Query Router Accuracy, CQ Pass Rate, Schema Compliance
  - Guardrail: Hallucination Rate ≤ 0.05 (ratio), Cache Hit Rate ≥ 0.70 (ratio) (위반 불가)
  - Diagnostic: CTE, KVCache Cost, VES, Deterministic Query Rate
- Guardrail 2회 연속 위반 → Opik 자동 알림
- North Star 3일 연속 하락 → Leading/Diagnostic 드릴다운 자동 트리거

## 안티패턴 (절대 하지 말 것)
- ❌ AI가 분석 결론/권고사항을 직접 생성 → 구조화된 질문과 체크리스트만 생성
- ❌ Quick/Full 모드 혼용 → 분석 시작 시 모드 확정, 중간 변경 시 새 분석 생성
- ❌ .analysis/ 파일을 .gitignore에 추가 → 반드시 Git 추적 (지식베이스 축적)
- ❌ SKILL.md를 CLAUDE.md 규칙과 병합 → 독립 파일로 유지, 충돌 시 CLAUDE.md 우선
- ❌ 비분석 세션에서 SKILL.md 상시 로드 → 토큰 낭비, /analysis-init 시에만 활성화
```
...
19. **캐시 히트율 모니터링**: SEOCHO API 운영 시 캐시 히트율 ≥ 70% 유지 확인. 도구 정의 순서 변경, 모델 전환, 시스템 프롬프트 수정 등 캐시 파괴 패턴 발견 시 `execution/known-issues.md`에 CRITICAL로 기록 (Implementation Strategy §22.6 참조)
20. **분석 파일 축적 (alive-analysis)**: ALIVE 루프 완료 시 `.analysis/` 디렉토리에 분석 파일 자동 커밋. 교차참조 탐지(동일 주제 기존 분석 자동 링크), 상충 발견 알림(기존 결론과 충돌 시 경고). 분석 파일은 절대 삭제 불가(아카이브만 허용). Quick→Full 승격 시 원본 Quick 파일도 보존 (Implementation Strategy §23 참조)
...
touch .claude/domains/{ontology-engine,nl2sql-pipeline,rag-search,agent-routing,analysis-workflow}.md
