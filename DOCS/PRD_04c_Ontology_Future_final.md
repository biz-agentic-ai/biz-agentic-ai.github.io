...
> 📌 본 섹션의 **Stage(0.5~3)**는 외부 데이터 파이프라인 내부 단계이며, 제품 로드맵 Phase와 1:1 대응하지 않습니다. 매핑: Stage 0.5~1 = Phase 2, Stage 2~3 = Phase 3. 상세 매핑은 §4.3.9.9를 참조하세요.
...
##### 4.3.10.10.3.1 프롬프트 캐싱 전제 조건 (Claude Code 실전 교훈 적용)

> **출처:** Anthropic Claude Code 팀 프롬프트 캐싱 운영 교훈 (2026.02)
> **상세 전략:** Implementation Strategy §22 참조

프롬프트 캐싱은 **프리픽스 매칭** 방식이므로, 프리픽스의 어떤 위치에서든 변경이 발생하면 해당 지점 이후의 모든 캐시가 무효화됩니다. 캐시 인식 프루닝이 효과를 발휘하려면 아래 전제 조건이 충족되어야 합니다.

**시스템 프롬프트 배치 순서 (Static First, Dynamic Last):**

| 배치 순서 | 구성 요소 | 캐시 범위 | 변경 빈도 |
|-----------|----------|----------|----------|
| 1 (최상단) | Static System Prompt + Tool Definitions | 전역 (모든 세션) | 배포 시에만 |
| 2 | Ontology Context + DDL Schema | 테넌트별 | 온톨로지 갱신 시 |
| 3 | Session Context (Graphiti Facts, User Prefs) | 세션별 | 세션 시작 시 |
| 4 (최하단) | Conversation Messages | 턴별 | 매 턴 |

**세션 중 절대 금지 항목:**

| 금지 항목 | 이유 | 대안 |
|-----------|------|------|
| 도구 추가/제거 | 도구 정의가 캐시 프리픽스의 일부 → 전체 캐시 무효화 | `defer_loading` 스텁 패턴: 경량 스텁 상시 포함, `ToolSearch`로 필요 시 전체 스키마 로딩 |
| 모델 전환 | 프롬프트 캐시는 모델별 고유 → 새 모델은 캐시 0에서 시작 | 서브에이전트 패턴: 메인 모델 유지, 간단한 작업은 서브에이전트에 핸드오프 |
| 시스템 프롬프트 수정 | 프리픽스 변경 → 이후 모든 캐시 미스 | `<system-reminder>` 태그를 다음 사용자 메시지에 삽입하여 정보 업데이트 |
| 도구 정의 순서 셔플 | 동일 도구라도 순서 변경 시 프리픽스 불일치 | `OrderedDict` 또는 정렬된 리스트로 순서 고정 |

**캐시 안전 컴팩션:** §4.3.10.10.6의 컴팩션 시 부모 대화와 **동일한 시스템 프롬프트, 도구 정의, 컨텍스트**를 사용하여 캐시 프리픽스를 재활용합니다. 별도 시스템 프롬프트를 사용하거나 도구를 제거하면 부모 세션의 캐시와 불일치하여 전체 입력 토큰에 대해 풀 프라이스를 지불하게 됩니다.
...
> **캐시 안전 컴팩션:** 컴팩션 API 호출 시 부모 대화와 동일한 시스템 프롬프트·도구 정의·컨텍스트를 사용하여 캐시 프리픽스를 재활용합니다. 별도 시스템 프롬프트나 도구 제거 시 부모 캐시가 무효화되어 전체 입력 토큰에 풀 프라이스가 발생합니다. (Implementation Strategy §22.4 참조)
