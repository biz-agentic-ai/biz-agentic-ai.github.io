...
>
> **짝꿍 문서:** [Implementation Guide](Implementation_Guide_final.md)
...
3.  **Quality Gate 통과 확인:** PRD 정의 임계값 - Query Router Accuracy ≥
    0.95, ConflictResolutionScore ≥ 0.95, Hallucination Rate ≤ 0.05

4.  **(선택) OMC Ralph 지속 실행:** Quality Gate 미달 시 ralph 루프로
    자동 수정 사이클을 돌립니다.

> ralph: \"Quality Gate 미달 항목 수정 - query_router_accuracy 현재 0.93,
> 목표 0.95\"
...
<!-- SSOT: 정식 버전은 CLAUDE_md_Template_final.md 참조. 본 섹션은 전략적 맥락만 기술 -->
...
### 16.7 Vanna Tool Memory ↔ Graphiti 이중 메모리 구현 가이드
...
## §18. KanVibe 태스크 관리 통합

> **📌 Phase 안내:** §18.1~18.2는 **Phase 1.0 MVP 착수 시점**에 도입합니다. §18.3 Cowork 연동은 Phase 2에서 구현합니다.
>
> **관련 문서:** KanVibe_Adoption_Review_for_DataNexus.md (Fit-Gap 분석 원본)
>
> **대상 리포지토리:** https://github.com/rookedsysc/kanvibe

### 18.1 도입 배경 및 정합성 개요

§6.2 Agent Teams 기반 병렬 개발에서 식별된 네 가지 구조적 제한사항을 해결합니다:

| 제한사항 | 현재 영향 | KanVibe 해소 방식 |
|-----------|-----------|-------------------|
| 세션 휘발성 — Claude Code 세션 종료 시 상태 유실 | PROGRESS.md 수동 갱신 누락 | PostgreSQL 영속 저장 + Hooks 자동 상태 전환 |
| 태스크 상태 지연 — 에이전트 응답 완료까지 상태 미갱신 | 실시간 모니터링 불가 | 5-Status 칸반 자동 전환 (Hooks 기반) |
| PROGRESS.md 수동 갱신 — 규칙은 있으나 강제력 부족 | 상태 불일치 누적 | KanVibe를 SSOT로 지정, API 동기화 |
| Worktree 수동 관리 — 생성/정리 스크립트 별도 실행 필요 | 환경 오염 리스크 | 태스크 생성/DONE 시 자동 생성/삭제 |

**종합 평가:** 4.2/5.0 (도입 권장) — Worktree 통합 ★5, Hooks 자동화 ★5, 인프라 호환성 ★5

### 18.2 통합 아키텍처

#### 18.2.1 5-Status 칸반 ↔ DataNexus 라이프사이클 매핑

| KanVibe 상태 | DataNexus 대응 | Hooks 자동 전환 트리거 |
|-------------|---------------|---------------------|
| **TODO** | 79% Rule 분해 완료, 실행 대기 | 태스크 생성 시 기본값 |
| **PROGRESS** | Teammate 활발 작업 중 | 사용자 프롬프트 입력 → Hooks 자동 전환 |
| **PENDING** | AskUser 대기 / 의존성 블록 | AI가 질문(AskUser) 시 → Hooks 자동 전환 |
| **REVIEW** | AI 작업 완료, Cross-Review 대기 | AI 응답 완료 시 → Hooks 자동 전환 |
| **DONE** | 커밋 완료 + 환경 자동 정리 | 수동 전환 후 Worktree/브랜치/세션 자동 삭제 |

#### 18.2.2 Worktree ↔ KanVibe 프로젝트 매핑

§12.2의 5개 Worktree가 KanVibe 태스크로 직접 매핑됩니다:

| Worktree (§12.2) | Teammate (§6.2) | KanVibe 태스크 접두사 |
|-------------------|-----------------|---------------------|
| `wt-backend` | Backend Core | `[Backend]` |
| `wt-graph` | Graph Engine | `[Graph]` |
| `wt-rag` | RAG Pipeline | `[RAG]` |
| `wt-agent` | Agent Logic | `[Agent]` |
| `wt-analysis` | (수동/별도) | `[Analysis]` |

**Agent Teams 다중 Teammate 조율:** KanVibe는 단일 태스크-단일 세션 모델이므로, 각 Teammate 작업을 독립 태스크로 등록하고 태스크명에 모듈 접두사를 사용하여 식별합니다.

#### 18.2.3 .claude/hooks/ 자동 설치 및 기존 구조 공존

KanVibe 프로젝트 등록 시 `.claude/hooks/` 디렉토리에 Hook 스크립트가 자동 설치됩니다. 기존 Context-as-Code 구조(§14)와의 경로 충돌은 없습니다:

```
.claude/
├── hooks/              ← KanVibe 자동 설치 (신규)
│   ├── start.sh
│   └── status.sh
├── rules/              ← 기존 유지 (§14 Layer 1)
├── foundation/         ← 기존 유지 (§14 Layer 2)
├── domains/            ← 기존 유지 (§14 Layer 2)
├── execution/          ← 기존 유지 (§14 Layer 3)
└── external-docs/      ← 기존 유지 (§15)
```
...
| 서비스 | DataNexus 포트 | KanVibe 포트 | 충돌 여부 |
|--------|---------------|-------------|----------|
| Neo4j Browser | 7474, 7687 | — | 없음 |
| FastAPI Agent Server | 8001 | — | 없음 |
| Streamlit Agent Studio | 8501 | — | 없음 |
| Opik Dashboard | 5173 | — | 없음 |
| DataHub UI | 9002 | — | 없음 |
| KanVibe Web | — | 4885 | 없음 |
| KanVibe PostgreSQL | — | 4886 | 없음 |

**결과:** 포트 충돌 없음. Docker Compose 동시 운영 가능.

#### 18.3.2 리소스 오버헤드

- PostgreSQL 16 컨테이너 1개 (~256MB)
- Next.js 서버 (~512MB)
- 총 추가 디스크: ~500MB (Docker 이미지 + 데이터)
- DataNexus 개발 환경 기준 미미한 오버헤드

### 18.4 PROGRESS.md 동기화

**원칙:** KanVibe를 SSOT(Single Source of Truth)로 지정하고, PROGRESS.md는 KanVibe API에서 동기화합니다.

```bash
# scripts/sync-progress.sh
#!/bin/bash
KANVIBE_URL="http://localhost:4885"
curl -s "${KANVIBE_URL}/api/tasks?project=datanexus" | \
  python3 -c "
import json, sys
tasks = json.load(sys.stdin)
sm = {'TODO':'[ ]','PROGRESS':'[~]','PENDING':'[?]','REVIEW':'[!]','DONE':'[x]'}
print('# DataNexus Progress (KanVibe Synced)')
for s in ['PROGRESS','PENDING','REVIEW','TODO','DONE']:
    g = [t for t in tasks if t['status']==s]
    if g:
        print(f'\n## {s} ({len(g)})')
        for t in g: print(f'- {sm[s]} {t["title"]}')
" > PROGRESS.md
```

### 18.5 Cowork Plugin 연동 (Phase 2)

§11.3 Cowork Plugin 커스터마이즈와 연계하여, Phase 2에서 다음 커스텀 명령어를 추가합니다:

| 명령어 | 기능 | 연동 API |
|--------|------|----------|
| `/dn:kanvibe-sync` | KanVibe → PROGRESS.md 즉시 동기화 | `GET /api/tasks` |
| `/dn:kanvibe-create` | 새 태스크 생성 (모듈 접두사 자동) | `POST /api/tasks` |
| `/dn:kanvibe-status` | 현재 칸반 상태 요약 출력 | `GET /api/hooks/status` |

### 18.6 영향 분석

| 영향 영역 | 변경 사항 | 영향도 |
|-----------|----------|--------|
| §12 Worktree 전략 | setup_worktrees.sh 보완 (KanVibe 자동 스캔 활용) | 낮음 |
| §13 Cross-Review | REVIEW 상태 태스크에 대해 Cross-Review 체크리스트 적용 | 낮음 |
| §14 CLAUDE.md | `.claude/hooks/` 디렉토리 추가, Accumulation Policy #14-15 | 낮음 |
| §6 PROGRESS.md | 수동 갱신 → sync-progress.sh 자동 생성으로 전환 | 중간 |
| 인프라 | ~768MB 추가 (PostgreSQL + Next.js) | 낮음 |

### 18.7 공수 요약

| Phase | 작업 | 공수 |
|-------|------|------|
| Phase 1.0 (§18.1–18.2) | KanVibe 설치, 프로젝트 등록, Hooks 확인, 동기화 스크립트 | 2 M/D |
| Phase 2.0 (§18.5) | Cowork Plugin 커스텀 명령어 3개 구현 | 1 M/D |
| **합계** | | **3 M/D** |
...
| 본 절 | 참조 대상 | 참조 방향 |
|--------|-----------|----------|
| §18.1 제한사항 | §6.2 Agent Teams | §6 → §18 해결 |
| §18.2.2 Worktree 매핑 | §12.2 Worktree 전략 | §12 ↔ §18 양방향 |
| §18.2.3 Hooks 공존 | §14 CLAUDE.md 아키텍처 | §14 ↔ §18 양방향 |
| §18.3 포트 검증 | §3 인프라 요구사항 | §3 → §18 검증 |
| §18.4 PROGRESS.md | §6 진행 추적 | §6 ← §18 대체 |
| §18.5 Cowork 연동 | §11.3 Cowork Plugin | §11 ← §18 확장 |
| §18.6 Cross-Review | §13.2 체크리스트 | §13 ← §18 연계 |
...
> **📌 Phase 안내:** §19.1은 **Phase 1.0** MVP 착수 시 도입합니다. §19.2 고급 파이프라인은 Phase 2에서 구현합니다.
>
> **출처:** Claude Code 70 Tips — Part 7.5 Headless Mode (Ado #30), Part 8 Multi-Agent (ykdojo #21)

### 19.1 Headless Mode PR 리뷰

Claude Code의 `claude -p "prompt"` 비대화형 모드를 GitHub Actions에 통합하여, PR 제출 시 DataNexus Quality Gate를 자동 검증합니다.
...
jobs:
  ai-review:
    ...
    steps:
      ...
      - name: Generate Diff
        run: git diff origin/main...HEAD > pr-diff.txt

      - name: Claude Quality Gate Review
        run: |
          cat pr-diff.txt | claude -p "
          DataNexus PR 리뷰를 수행하세요. 다음 Quality Gate를 검증:
          1. Ontology: SKOS 스키마 준수, URN 변환 규칙 확인
          2. NL2SQL: Forbidden Tables 접근 여부, SQL injection 패턴
          3. Agent: Graph DBA의 DELETE/DROP 포함 여부
          4. Code: Pydantic V2 스키마 존재, Type hints 확인
          5. Test: 새 로직에 대한 테스트 파일 존재 여부
          JSON 형식으로 출력: {gate, status, issues[]}
          " > review-result.json
      ...
      - name: Post Review Comment
        ...
        with:
          script: |
            const fs = require('fs');
            const result = fs.readFileSync('review-result.json', 'utf8');
            github.rest.issues.createComment({
              owner: context.repo.owner,
              repo: context.repo.repo,
              issue_number: context.issue.number,
              body: '## 🤖 Claude Quality Gate Review\n```json\n' + result + '\n```'
            });
```

#### 19.1.2 검증 항목 매핑

| Quality Gate | 검증 대상 파일 패턴 | 차단 레벨 |
|-------------|-------------------|----------|
| Ontology Schema | `ontology/**/*.ttl`, `*.skos` | CRITICAL (머지 차단) |
| Forbidden Tables | `src/**/sql/**`, `*.cypher` | CRITICAL (머지 차단) |
| Pydantic V2 | `src/**/models.py`, `**/schemas.py` | WARNING (리뷰 코멘트) |
| Test Coverage | `tests/**` 신규 파일 존재 | WARNING (리뷰 코멘트) |
| Agent Safety | `src/agents/**` | CRITICAL (머지 차단) |

### 19.2 고급 파이프라인 (Phase 2)

- **자동 온톨로지 SHACL 검증**: PR에 온톨로지 변경 포함 시, SHACL Shape 검증을 자동 실행
- **Vanna SQL 정확도 회귀 테스트**: 변경된 DDL에 대해 기존 Few-shot 30쌍 재검증
- **보안 감사 통합**: `npx cc-safe .` 결과를 PR 코멘트에 포함

### 19.3 공수 요약

| 항목 | Phase | 담당 | 공수 |
|------|-------|------|------|
| GitHub Actions 워크플로우 설정 | Phase 1.0 | Backend Core | 1 M/D |
| 고급 파이프라인 (SHACL + Vanna) | Phase 2.0 | Graph Engine + Backend | 2 M/D |
| **합계** | | | **3 M/D** |
...
> **📌 Phase 안내:** §20.1 기본 패턴은 **Phase 1.0**에서 도입합니다. §20.2 자동 오케스트레이션은 Phase 2에서 구현합니다.
>
> **출처:** Claude Code 70 Tips — Part 8.1 Multi-Agent Orchestration (ykdojo #21)

### 20.1 기본 패턴: Main → Worker 위임

DDL 분석, SKOS 변환, 그래프 대량 로딩 등 잠재적으로 위험하거나 장시간 소요되는 작업을 Docker 컨테이너 안의 Worker Claude에 위임합니다.
...
│  Main Claude (Host)                         │
│  - 작업 분해 + 결과 검증                      │
│  - Forbidden Tables / Agent Rules 적용       │
│  - PROGRESS.md / KanVibe 갱신               │
│                                             │
│  ┌─── Docker Container ──────────────────┐  │
│  │  Worker Claude                        │  │
...
│  │  - Cypher UNWIND 배치 로딩             │  │
│  │  - 대용량 임베딩 생성                   │  │
│  │  ※ Guardian Hook은 컨테이너 내부에서도   │  │
│  │    Graph DBA DELETE/DROP 차단 유지      │  │
│  └───────────────────────────────────────┘  │
│                                             │
│  결과물: /output/ 마운트 → Host로 반환        │
└─────────────────────────────────────────────┘
```
...
| 안전장치 | 설명 | 구현 |
|---------|------|------|
| Hook 이중 차단 | `--dangerously-skip-permissions`에서도 DELETE/DROP 차단 | `.claude/hooks/` 컨테이너 내부 마운트 |
| 네트워크 격리 | 컨테이너는 DozerDB/Qdrant만 접근 가능 | Docker network 제한 |
| 볼륨 제한 | `/output/`만 Host에 쓰기 가능 | read-only bind mount |
| 타임아웃 | 최대 30분, 초과 시 컨테이너 강제 종료 | `docker run --timeout 1800` |
| 결과 검증 | Worker 산출물을 Main이 Pydantic V2로 검증 후 수용 | Main Claude 후처리 |

#### 20.1.3 활용 시나리오

| 시나리오 | Worker 작업 | 예상 소요 | 위험도 |
|---------|------------|----------|--------|
| DDL → SKOS 변환 | 테이블 100+개 DDL 파싱 → RDF Turtle 생성 | 5-15분 | 중 |
| 그래프 배치 로딩 | Cypher UNWIND 1000행 × N 배치 | 10-20분 | 고 |
| 임베딩 대량 생성 | ApeRAG 문서 500+ 청크 벡터화 | 15-30분 | 저 |
| Few-shot SQL 검증 | Vanna 30쌍 일괄 실행 + 정확도 측정 | 5-10분 | 중 |

### 20.2 자동 오케스트레이션 (Phase 2)

Agent Teams의 Teammate가 Worker 컨테이너를 직접 스폰하는 패턴:

- **Graph Engine Teammate**: 온톨로지 대량 변경 감지 시 자동으로 Worker 생성
- **Backend Core Teammate**: DDL 변경 감지 시 Vanna 재학습 Worker 생성
- **tmux 세션 관리**: 각 Worker를 tmux 패널로 관리, Main에서 실시간 모니터링

### 20.3 공수 요약

| 항목 | Phase | 담당 | 공수 |
|------|-------|------|------|
| Docker 워커 이미지 + Hook 마운트 | Phase 1.0 | Backend Core | 1 M/D |
| 자동 오케스트레이션 (Teammate → Worker) | Phase 2.0 | Agent Logic | 2 M/D |
| **합계** | | | **3 M/D** |


---

## §21. DataNexus Skills & Commands 체계

> **📌 Phase 안내:** §21.1 Slash Commands는 **Phase 1.0**에서 도입합니다. §21.2 Skills(자동 호출)는 Phase 2에서 구현합니다.
>
> **출처:** Claude Code 70 Tips — Part 6.6 Skills vs Slash Commands (ykdojo #25)

### 21.1 개념 구분: Skills vs Slash Commands

| 구분 | Skills (자동 호출) | Slash Commands (수동 호출) |
|------|-------------------|-------------------------|
| 트리거 | Claude가 컨텍스트에서 자동 감지 | 개발자가 `/명령어` 직접 입력 |
| 위치 | `.claude/skills/` | `.claude/commands/` |
| 실행 주체 | Claude Code 에이전트 | 개발자 의도적 호출 |
| DataNexus 용도 | 반복적 검증·보정 자동화 | 상태 조회·내보내기·전환 작업 |

### 21.2 DataNexus Slash Commands (Phase 1.0)

#### 21.2.1 명령어 목록

| 명령어 | 기능 | 구현 |
|--------|------|------|
| `/datanexus:handoff` | HANDOFF.md 표준 템플릿으로 생성 + handoff-log 기록 | CLAUDE.md 템플릿 참조 |
| `/datanexus:quality-gate` | 현재 모듈의 Quality Gate 상태 출력 (5개 메트릭) | PRD_05 §5 기준 |
| `/datanexus:export-ontology` | 현재 온톨로지 상태를 SKOS RDF Turtle로 내보내기 | DozerDB → RDF 변환 |
| `/datanexus:context-check` | `/context` + 토큰 예산 대비 초과 항목 자동 분석 | CLAUDE.md 기준선 참조 |
| `/datanexus:kanvibe-sync` | KanVibe → PROGRESS.md 즉시 동기화 | §18.5 연계 |

#### 21.2.2 구현 예시 (`/datanexus:handoff`)

```markdown
<!-- .claude/commands/datanexus-handoff.md -->
# HANDOFF.md 생성

현재 세션의 작업 상태를 HANDOFF.md로 정리합니다.

1. 현재 작업 디렉토리의 git status 확인
2. 최근 5개 커밋 메시지 수집
3. CLAUDE.md의 HANDOFF.md 표준 템플릿에 따라 생성
4. execution/handoff-log.md에 기록 추가
5. 생성된 HANDOFF.md 경로를 출력
```

### 21.3 DataNexus Skills (Phase 2)

Claude가 컨텍스트에서 자동으로 감지하여 호출하는 검증·보정 루틴입니다.

| Skill 이름 | 자동 트리거 조건 | 실행 내용 |
|-----------|----------------|----------|
| `ontology-validation` | 새 GlossaryTerm 추가 감지 | SHACL Shape 검증 + URN 규칙 확인 |
| `nl2sql-test` | SQL 생성 완료 감지 | Forbidden Tables 체크 + LIMIT 강제 + 구문 검증 |
| `conflict-resolution` | ConflictResolutionScore < 0.5 감지 | HoT 계층에서 최고 우선순위 소스 자동 선택 |
| `context-budget` | 세션 토큰 50% 도달 감지 | 토큰 예산 테이블 대비 초과 경고 |
| `anti-pattern-guard` | Domain Rules 안티패턴 매칭 감지 | 해당 안티패턴 경고 + 대안 제시 |

### 21.4 파일 구조

```
.claude/
├── commands/                 ← Slash Commands (Phase 1.0)
│   ├── datanexus-handoff.md
│   ├── datanexus-quality-gate.md
│   ├── datanexus-export-ontology.md
│   ├── datanexus-context-check.md
│   └── datanexus-kanvibe-sync.md
├── skills/                   ← Skills (Phase 2)
│   ├── ontology-validation.md
│   ├── nl2sql-test.md
│   ├── conflict-resolution.md
│   ├── context-budget.md
│   └── anti-pattern-guard.md
├── rules/                    ← 기존 유지
├── foundation/               ← 기존 유지
├── domains/                  ← 기존 유지
├── hooks/                    ← KanVibe 자동 설치 (§18)
├── execution/                ← 기존 유지
└── external-docs/            ← 기존 유지
```
...
| 영향 영역 | 변경 사항 | 영향도 |
|-----------|----------|--------|
| §14 CLAUDE.md 아키텍처 | `commands/`, `skills/` 디렉토리 추가 | 낮음 |
| §8 Context-as-Code | Skills가 Layer 2(Domain)의 자동 실행 확장 | 중간 |
| §17 개발 환경 보안 | `/datanexus:context-check`가 §17.2 모니터링 자동화 | 낮음 |
| §18 KanVibe | `/datanexus:kanvibe-sync`가 §18.5 Cowork 명령어와 통합 | 낮음 |
| 토큰 예산 | commands/ 5파일 (~500 tokens), skills/ 5파일 (~500 tokens) | 기준선 이내 |

### 21.6 공수 요약

| 항목 | Phase | 담당 | 공수 |
|------|-------|------|------|
| Slash Commands 5개 구현 | Phase 1.0 | Team Lead | 1 M/D |
| Skills 5개 구현 + 자동 트리거 | Phase 2.0 | Agent Logic | 2 M/D |
| **합계** | | | **3 M/D** |
...
| 본 절 | 참조 대상 | 참조 방향 |
|--------|-----------|----------|
| §19.1 Headless Mode | §5.3.5 테스트 파이프라인 | §5 ← §19 확장 |
| §19.1.2 검증 항목 | §17.1 cc-safe, §14 Quality Gates | §17 ← §19 통합 |
| §20.1 Worker 패턴 | §6.2 Agent Teams, §12 Worktree | §6/§12 ← §20 확장 |
| §20.1.2 Hook 이중 차단 | §14 Guardian Hook | §14 ↔ §20 양방향 |
| §21.2 Slash Commands | §14 CLAUDE.md, §18.5 Cowork | §14/§18 ← §21 확장 |
| §21.3 Skills | §8 Context-as-Code, §17.2 모니터링 | §8/§17 ← §21 자동화 |
| §22 Prompt Caching | §4.3.10.10.3 캐시 인식 프루닝, §5.1 Cache Hit Rate | PRD_04c/PRD_05 ← §22 전략 |

---

## 22. Prompt Caching Strategy (SEOCHO API 최적화)

> **📌 출처:** Anthropic Claude Code 팀의 프롬프트 캐싱 실전 교훈 (2026.02.20, Thariq @trq212)
> **적용 대상:** SEOCHO 프로덕션 에이전트의 Anthropic API 호출 최적화
> **관련 PRD:** PRD_04c §4.3.10.10.3 (캐시 인식 프루닝), PRD_05 §5.1 (Cache Hit Rate ≥ 70%)

### 22.1 배경 및 핵심 원칙

프롬프트 캐싱은 **프리픽스 매칭(prefix matching)** 방식으로 동작합니다. API가 요청의 시작부터 각 `cache_control` 브레이크포인트까지 캐시하므로, 프리픽스의 **어떤 위치에서든 변경이 발생하면 그 이후의 모든 캐시가 무효화**됩니다. SEOCHO의 대형 온톨로지 컨텍스트(~8K) + DDL 스키마(~6K) + HoT 규칙(~5K) = ~19K tokens이 매 턴 재처리되는 비용을 캐시 히트로 절감하는 것이 핵심입니다.

Claude Code 팀은 캐시 히트율을 **가동률(uptime)처럼 모니터링**하며, 캐시 히트율 저하 시 SEV(장애 등급)를 선언합니다. DataNexus도 동일한 수준의 관리를 적용합니다.

### 22.2 시스템 프롬프트 배치 순서 (Static First, Dynamic Last)

SEOCHO 에이전트의 프롬프트 요소를 캐시 효율성 최대화 순서로 배치합니다. **가능한 많은 세션이 동일 프리픽스를 공유**하도록 정적 컨텐츠를 앞에, 동적 컨텐츠를 뒤에 배치합니다.

```txt
┌─────────────────────────────────────────────────────────────────┐
│  SEOCHO Prompt Layout for Cache Optimization                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  [Layer 1] Static System Prompt + Tool Definitions                │
│            → 전역 캐시 (모든 세션 공유)                            │
│            포함: 에이전트 역할 정의, HoT 규칙, 금지 패턴           │
│            ⚠️ 도구 정의 순서 고정 필수 (비결정적 셔플 금지)        │
│                                                                   │
│  [Layer 2] Ontology Context + DDL Schema                          │
│            → 테넌트별 캐시 (동일 테넌트 세션 공유)                 │
│            포함: DataHub Glossary 컨텍스트, 테이블 스키마           │
│                                                                   │
│  [Layer 3] Session Context (Graphiti Facts, User Preferences)     │
│            → 세션별 캐시 (동일 세션 내 턴 간 공유)                 │
│            포함: 사용자 페르소나, 최근 대화 사실                    │
│                                                                   │
...
| 위험 요소 | 문제 | 해결 방안 |
|-----------|------|----------|
| 시스템 프롬프트 내 상세 타임스탬프 | 매 요청 프리픽스 변경 → 전체 캐시 미스 | 타임스탬프는 `<system-reminder>` 태그로 대화 메시지에 주입 |
| 도구 정의 순서 비결정적 셔플 | Dict/Set 순회 시 순서 비보장 → 캐시 미스 | `OrderedDict` 또는 정렬된 리스트로 도구 정의 순서 고정 |
| 도구 파라미터 동적 변경 | 에이전트 호출 가능 목록 런타임 변경 → 캐시 미스 | 도구 파라미터를 정적으로 고정, 변경은 메시지로 전달 |

### 22.3 세션 중 도구/모델 변경 금지 원칙

#### 22.3.1 도구 변경 금지

도구 세트(tool set)는 캐시 프리픽스의 일부이므로, **세션 중간에 도구를 추가/제거하면 전체 대화의 캐시가 무효화**됩니다.

**SEOCHO 적용 방안:**

| 시나리오 | ❌ 직관적 접근 (캐시 파괴) | ✅ 캐시 안전 접근 |
|---------|--------------------------|------------------|
| Plan Mode 진입 | 읽기 전용 도구만 남기고 제거 | `EnterPlanMode`/`ExitPlanMode`를 도구로 정의, 시스템 메시지로 모드 전환 |
| 에이전트별 도구 제한 | Router에는 분류 도구만 제공 | 모든 도구를 항상 포함, 에이전트별 권한은 시스템 프롬프트로 제어 |
| MCP 도구 동적 로딩 | 필요 시 MCP 도구 추가 | `defer_loading` 스텁 패턴으로 경량 스텁만 상시 포함 |

**defer_loading 스텁 패턴 (MCP 도구 확장):**

MCP 도구가 다수일 때 모든 스키마를 프롬프트에 포함하면 비용이 급증합니다. Claude Code의 `defer_loading` 패턴을 차용하여, 도구 이름만 포함한 경량 스텁을 상시 포함하고, 모델이 `ToolSearch` 도구로 필요한 도구를 발견하면 전체 스키마를 로딩합니다.

```python
# defer_loading 스텁 예시
TOOL_STUBS = [
    {"name": "graphiti_search", "defer_loading": True},
    {"name": "datahub_sync", "defer_loading": True},
    {"name": "shaper_dashboard", "defer_loading": True},
]

# 항상 활성화되는 핵심 도구
CORE_TOOLS = [
    full_schema("nl2sql_generate"),
    full_schema("graph_query"),
    full_schema("vector_search"),
    full_schema("tool_search"),  # 스텁 도구를 발견하는 메타 도구
]

# API 요청 시: CORE_TOOLS + TOOL_STUBS를 항상 동일 순서로 전달
```

#### 22.3.2 모델 변경 금지

프롬프트 캐시는 모델별로 고유합니다. 100K 토큰 대화 중 "간단한 질문"이라고 Haiku로 전환하면, Haiku를 위한 캐시를 처음부터 구축해야 하므로 오히려 비용이 증가합니다.

**SEOCHO 적용:**

| 시나리오 | 해결 방안 |
|---------|----------|
| 간단한 분류/요약 작업 | 메인 세션 모델(Sonnet)을 유지, 서브에이전트에서 Haiku 사용 |
| 복잡한 추론 필요 시 | 메인 세션에서 Opus로 전환하지 않고, Opus 서브에이전트에 "핸드오프 메시지" 전달 |
| Explore/검증 작업 | Claude Code의 Explore Agent 패턴 차용: Haiku 서브에이전트에 축약된 컨텍스트 전달 |

### 22.4 캐시 안전 컴팩션 (Cache-Safe Compaction)

PRD_04c §4.3.10.10.6의 컴팩션 전 Graphiti 메모리 플러시와 연계하여, 컴팩션 API 호출 시에도 캐시를 최대한 재활용합니다.

**핵심 원칙:** 컴팩션 요약을 생성할 때 **부모 대화와 동일한 시스템 프롬프트, 도구 정의, 컨텍스트**를 사용하여 캐시 프리픽스를 재활용합니다.
...
│  [1] 컨텍스트 사용률 70% 도달                                  │
│       │                                                       │
│  [2] Graphiti 메모리 플러시 (§4.3.10.10.6)                    │
│       │  ← 핵심 사실을 에피소드로 영구 저장                     │
│       │                                                       │
│  [3] 컴팩션 API 호출                                           │
│       │  ★ 동일 시스템 프롬프트 사용 (캐시 재활용)              │
│       │  ★ 동일 도구 정의 사용 (캐시 재활용)                   │
│       │  ★ 부모 대화 메시지 선행 + 컴팩션 프롬프트 후행         │
│       │                                                       │
│  [4] 요약 생성 → 새 세션 시작                                  │
│       │  ★ 컴팩션 버퍼 확보 (요약 출력 토큰 여유 공간)          │
│                                                               │
│  비용 절감: 부모 세션의 캐시 프리픽스 재활용으로                 │
│            컴팩션 시 입력 토큰 비용 ~0 (캐시 히트)              │
└─────────────────────────────────────────────────────────────┘
```
...
| 항목 | 값 | 설명 |
|------|---|------|
| 컴팩션 트리거 | 컨텍스트 사용률 70% | PRD_04c §4.3.10.10.6과 동일 |
| 컴팩션 버퍼 | 20% (200K 기준 ~40K tokens) | 요약 프롬프트 + 요약 출력 공간 |
| 안전 마진 | 10% | Graphiti 플러시 + 예상치 못한 도구 결과 |

**❌ 안티패턴:** 컴팩션 시 별도 시스템 프롬프트를 사용하거나, 도구를 제거하면 부모 세션의 캐시와 불일치하여 전체 입력 토큰에 대해 풀 프라이스를 지불하게 됩니다.

### 22.5 시스템 메시지를 통한 상태 업데이트

시스템 프롬프트를 변경하는 대신 `<system-reminder>` 태그를 다음 사용자 메시지 또는 tool_result에 삽입하여 정보를 업데이트합니다.

**적용 사례:**

| 업데이트 유형 | ❌ 시스템 프롬프트 변경 | ✅ system-reminder 메시지 |
|-------------|----------------------|-------------------------|
| 현재 시각 | 시스템 프롬프트에 타임스탬프 삽입 | `<system-reminder>현재 시각: 2026-02-20 14:30 KST</system-reminder>` |
| 사용자 파일 변경 | 스키마 컨텍스트 재구성 | `<system-reminder>사용자가 DDL 스키마를 수정했습니다: ALTER TABLE...</system-reminder>` |
| Plan Mode 진입 | 도구 세트 교체 | `<system-reminder>Plan Mode 활성화: 읽기 전용 모드로 탐색하세요. ExitPlanMode 도구로 종료.</system-reminder>` |
| 에이전트 권한 변경 | 도구 파라미터 수정 | `<system-reminder>사용자 권한 업데이트: financial_db 접근 허용됨</system-reminder>` |

### 22.6 캐시 히트율 모니터링 및 알림

> **SSOT 연계:** PRD_05 §5.1.3 Phase 2 지표 "Cache Hit Rate ≥ 70%"

**모니터링 체계:**

| 레벨 | Cache Hit Rate | 동작 |
|------|---------------|------|
| 정상 | ≥ 80% | 정상 운영 |
| 경고 | 70~80% | Opik 대시보드 알림, 원인 분석 |
| 위험 | 60~70% | SEV-3 선언, 즉시 원인 조사 |
| 장애 | < 60% | SEV-2 선언, 긴급 패치 |

**캐시 미스 주요 원인 및 디버깅:**

| 원인 | 탐지 방법 | 해결 |
|------|----------|------|
| 도구 정의 순서 변경 | 연속 요청 간 도구 JSON diff | `OrderedDict` 적용 + 정렬 보장 |
| 온톨로지 컨텍스트 갱신 | 캐시 미스 시점과 DataHub Sync 시점 대조 | 온톨로지 갱신을 세션 시작 시점으로 제한 |
| MCP 도구 동적 추가 | `defer_loading` 미적용 도구 식별 | 모든 MCP 도구에 스텁 패턴 적용 |
| 모델 전환 | API 로그에서 모델 변경 추적 | 서브에이전트 패턴으로 대체 |

### 22.7 Phase별 적용 계획

| Phase | 적용 항목 | 예상 효과 |
|-------|----------|----------|
| **Phase 1 (MVP)** | §22.2 시스템 프롬프트 배치 순서 고정 | 기본 캐시 인프라 확보 |
| **Phase 1 (MVP)** | §22.3.2 모델 변경 금지 원칙 적용 | 불필요한 캐시 미스 방지 |
| **Phase 2** | §22.3.1 defer_loading 스텁 패턴 도입 | MCP 도구 확장 시 캐시 안정성 |
| **Phase 2** | §22.4 캐시 안전 컴팩션 구현 | 컴팩션 비용 ~80% 절감 |
| **Phase 2** | §22.5 system-reminder 패턴 적용 | 런타임 업데이트 시 캐시 보존 |
| **Phase 2** | §22.6 캐시 히트율 모니터링 대시보드 | 운영 가시성 확보 |

### 22.8 비용 영향 분석

**현재 (캐시 미적용):** 온톨로지(~8K) + DDL(~6K) + HoT 규칙(~5K) + 도구 정의(~10K) = ~29K tokens/턴 × 풀 프라이스

**목표 (캐시 적용):** 동일 세션 내 2번째 턴부터 ~29K tokens를 캐시 히트로 처리 → Anthropic 캐시 히트 가격(입력 대비 ~10%) 적용

**예상 비용 절감:** 평균 세션 10턴 기준, 입력 토큰 비용 **~80% 절감** (PRD_04c §4.3.10.10.3 추정치와 일치)

### 22.9 교차 참조

| 본 절 | 참조 대상 | 참조 내용 |
|--------|-----------|----------|
| §22.2 프롬프트 배치 | PRD_04c §4.3.10.10.3 | 캐시 인식 프루닝의 전제 조건 |
| §22.3 도구/모델 변경 금지 | CLAUDE.md Template agent-routing.md | 안티패턴으로 등록 |
| §22.4 캐시 안전 컴팩션 | PRD_04c §4.3.10.10.6 | Graphiti 메모리 플러시와 연계 |
| §22.5 system-reminder | PRD_02 §3.5 SEOCHO Agent | 에이전트 상태 전환 패턴 |
| §22.6 캐시 모니터링 | PRD_05 §5.1.3 Cache Hit Rate | Phase 2 지표 달성 전략 |

---

## §23. alive-analysis 구조화된 분석 통합

> **📌 이 섹션은 DataNexus의 데이터 분석 워크플로우에 alive-analysis ALIVE 루프를 통합하는 전략을 기술합니다.**
> - **짝꿍 섹션:** Implementation Guide STEP 24 (설치 및 실행 가이드)
> - **기능 명세:** PRD_02 §3.11 (구조화된 분석 워크플로우)
> - **평가 연계:** PRD_05 §5.1.5 (메트릭 4단계 분류 매핑)

### 23.1 도입 배경

DataNexus는 NL2SQL(Vanna), GraphRAG(ApeRAG), Dashboard Promotion(Shaper)으로 **데이터 접근 → 시각화 → 보고**의 파이프라인을 구축했다. 그러나 "검색된 데이터를 어떻게 해석하고, 어떤 결론을 도출할 것인가"의 **분석 추론 과정**은 사용자 개인의 역량에 의존하는 비구조화 영역으로 남아 있었다.

**alive-analysis**(https://github.com/with-geun/alive-analysis)는 이 빈 공간을 채우는 오픈소스 분석 워크플로우 킷이다. SKILL.md 기반으로 AI 코딩 에이전트(Claude Code, Cursor)에 구조화된 분석 사고 프레임워크를 주입한다. MIT 라이선스, Shell 100%, 커밋 28개, 기여자 2명, 전통적 의존성 없음.

**도입 정당성:**
1. DataNexus의 방어선 전략(PRD_01 §1)에서 "데이터 축적 속도 > 모델 일반화 속도"를 달성하려면, 데이터 접근뿐 아니라 **분석 추론 패턴의 축적**이 필요
2. alive-analysis의 Git 추적 분석 파일은 조직의 분석 지식베이스를 자동으로 구축하여 방어선에 직접 기여
3. A/B 실험 모듈과 메트릭 모니터링은 DataNexus의 Ablation Study(PRD_05 §5.4) 품질을 표준화
...
│                                                          │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌─────────┐ │
│  │  Vanna   │  │  ApeRAG  │  │  DozerDB │  │ Shaper  │ │
│  │ (NL2SQL) │  │(GraphRAG)│  │ (Graph)  │  │  (BI)   │ │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬────┘ │
│       │              │              │              │      │
│       └──────────────┼──────────────┘              │      │
│                      │                             │      │
...
│                      │                             │      │
│  ┌───────────────────▼─────────────────────────────┤      │
│  │         alive-analysis ALIVE Loop               │      │
│  │  ┌─────┐ ┌──────┐ ┌─────────────┐ ┌───────┐   │      │
│  │  │ ASK │→│ LOOK │→│ INVESTIGATE │→│ VOICE │→  │      │
│  │  └─────┘ └──────┘ └─────────────┘ └───────┘   │      │
│  │  ┌────────┐                                    │      │
│  │  │ EVOLVE │→ .analysis/ (Git-tracked)          │      │
│  │  └────────┘                                    │      │
│  ├────────────────────────────────────────────────┤      │
│  │  Experiment Module    │  Monitoring Module      │      │
│  │  (A/B Test Lifecycle) │  (4-tier Metrics)       │      │
│  └───────────────────────┴────────────────────────┘      │
│                                                          │
│  ┌──────────────────────────────────────────────────┐    │
│  │  Opik Evaluation Tracking                         │    │
│  │  (실험 결과 + 메트릭 히스토리 통합 대시보드)       │    │
│  └──────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────┘
```

**핵심 설계 결정:**
- alive-analysis는 DataNexus 코어 의존성이 아닌 **선택적 확장(Optional Extension)**으로 위치
- SKILL.md는 `.analysis/` 디렉토리에 설치되며, `.claude/` 디렉토리의 CLAUDE.md와 독립적으로 작동
- 분석 산출물(마크다운 파일)은 프로젝트 레포에 Git 추적되어 지식베이스 축적

### 23.3 SKILL.md와 CLAUDE.md 공존 규칙

| 항목 | CLAUDE.md | alive-analysis SKILL.md |
|------|-----------|------------------------|
| **범위** | DataNexus 프로젝트 전역 규칙 | 분석 도메인 특화 규칙 |
| **위치** | `.claude/rules/datanexus.md` | `.analysis/` 내 또는 `platforms/claude-code/SKILL.md` |
| **로딩** | Claude Code 세션 시작 시 자동 | `/analysis-init` 실행 시 활성화 |
| **토큰 비용** | ≤ 24,000 tokens (기존 예산) | 추가 ~4,000-5,000 tokens (SKILL.md 크기) |
| **충돌 해결** | CLAUDE.md 규칙 우선 | 분석 도메인 내에서는 SKILL.md 우선 |

**프롬프트 토큰 예산 영향:**
- alive-analysis SKILL.md 추가 시 전체 규칙 토큰이 ~28,000-29,000 tokens로 증가 (기존 24,000 대비 +17-21%)
- 대응: 분석 세션에서만 SKILL.md를 로드하는 조건부 활성화 전략 적용
- `/analysis-init` 실행 전까지 SKILL.md 미로딩 → 비분석 작업 시 토큰 절약

### 23.4 실험/모니터링 모듈 활용 전략

DataNexus의 Ablation Study(PRD_05 §5.4) 실험 M1-M4, A1-A6에 alive-analysis 실험 모듈을 적용:

| alive-analysis 실험 단계 | Ablation Study 적용 |
|-------------------------|-------------------|
| **Design** | 실험 가설, 변수 통제, 기대 효과를 `/experiment-new`로 문서화 |
| **Validate** | SRM 검사로 데이터셋 분할 비율 검증, 가드레일 메트릭 설정 |
| **Analyze** | Opik Trace 결과를 ALIVE 루프 INVESTIGATE 단계에서 구조화 분석 |
| **Decide** | 결과 기반 의사결정을 VOICE 단계에서 이해관계자별 메시징 |
| **Learn** | `/experiment-archive`로 실험 결과 아카이브, 교훈을 EVOLVE에 기록 |

### 23.5 Phase별 적용 계획

| Phase | 작업 | 산출물 | 의존성 |
|-------|------|-------|-------|
| **Phase 1.5** | alive-analysis 설치, Quick 모드 기본 워크플로우, `.analysis/` 디렉토리 초기화, Investigation 유형 우선 적용 | 기본 SKILL.md 커스터마이징 (DataNexus 도메인 용어 추가) | Vanna NL2SQL 기본 동작 (Phase 1.0) |
| **Phase 2.0** | Full 모드 활성화, 실험/모니터링 모듈 Opik 연계, Modeling/Simulation 유형 추가, 4단계 메트릭 대시보드 | Ablation Study 사전등록 자동화, 메트릭 에스컬레이션 파이프라인 | Opik 평가 추적 (Phase 1.0), Shaper 대시보드 (Phase 1.5) |
| **Phase 3+** | Graphiti 시간축 메모리와 분석 지식베이스 통합, 도메인별 커스텀 SKILL.md, Education 모듈 활용 온보딩 | 시계열 분석 인사이트 누적, 분석 역량 졸업 경로 | Graphiti (Phase 3) |

### 23.6 영향 분석

| 기존 컴포넌트 | 영향 | 호환성 |
|-------------|------|-------|
| `.claude/` 디렉토리 | 독립 — `.analysis/` 별도 디렉토리 사용 | ✅ 충돌 없음 |
| CLAUDE.md 토큰 예산 | 분석 세션 시 +4,000-5,000 tokens | ⚠️ 조건부 로딩으로 대응 |
| KanVibe 태스크 관리 | 분석 태스크에 `[Analysis]` 접두사 추가 | ✅ 기존 네이밍 규칙 확장 |
| Shaper Dashboard | Voice 단계에서 Promotion 연계 | ✅ §3.9 워크플로우 재사용 |
| Opik 평가 추적 | 실험/모니터링 결과 통합 | ✅ 기존 Trace 구조 활용 |
| Git 워크플로우 | `.analysis/` 디렉토리 Git 추적 추가 | ✅ .gitignore 미포함 확인 필요 |

### 23.7 공수 요약

| 작업 | 예상 공수 | Phase |
|------|----------|-------|
| alive-analysis 설치 및 SKILL.md 커스터마이징 | 0.5일 | 1.5 |
| `.analysis/` 디렉토리 초기화 + Git 설정 | 0.5일 | 1.5 |
| Quick 모드 Investigation 워크플로우 검증 | 1일 | 1.5 |
| CLAUDE.md 분석 도메인 규칙 추가 | 0.5일 | 1.5 |
| Full 모드 + 실험/모니터링 Opik 연계 | 2일 | 2.0 |
| 4단계 메트릭 대시보드 설정 | 1일 | 2.0 |
| **합계** | **5.5일** | |
...
|----------|------|------|
| 기능 명세 | PRD_02 | §3.11 |
| 컴포넌트 등록 | PRD_01 | §1 주요 구성 요소 |
| 평가 연계 | PRD_05 | §5.1.5 (메트릭 매핑), §5.4 (Ablation Study) |
| 리소스 URL | PRD_06 | §8.12 |
| 로드맵 | PRD_06 | §9.1, §9.3, §9.4 |
| 향후 검토 | PRD_06 | §10.2 |
| 설치 가이드 | Implementation Guide | STEP 24 |
| 도메인 규칙 | CLAUDE.md Template | analysis-workflow.md |
| 방어선 전략 | PRD_01 | §1 전략적 포지셔닝 |
| Dashboard Promotion | PRD_02 | §3.9 |
| 5개 페르소나 | PRD_07 | §11.12.3 |
| KanVibe 태스크 | Implementation Strategy | §18 |
| Prompt Caching | Implementation Strategy | §22 |
