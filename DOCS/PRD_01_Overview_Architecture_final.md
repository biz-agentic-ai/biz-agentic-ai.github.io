...
| 구조화된 분석 추론 축적 | alive-analysis ALIVE 루프 + 실험/모니터링 모듈 | Git 추적 분석 파일의 지식베이스 축적, 분석 패턴 재사용 |
...
DataNexus는 여섯 가지 핵심 오픈소스 솔루션을 조합하여 구성됩니다:
...
| **Shaper** | v0.12.9 | SQL 기반 대시보드 & 자동화 보고 | DuckDB 기반, PDF/CSV/Excel 리포트, 임베디드 분석(React SDK), Git 워크플로 |
| **alive-analysis** | v1.1 | 구조화된 데이터 분석 워크플로우 킷 | ALIVE 루프 5단계 (Ask→Look→Investigate→Voice→Evolve), SKILL.md 기반 20개 슬래시 커맨드, A/B 실험·메트릭 모니터링, MIT 라이선스 |
...
#### Shaper v0.12.9 – SQL 기반 대시보드 & 자동화 보고 (DuckDB 기반 BI)
DuckDB를 기반으로 SQL만으로 데이터 대시보드를 구성하고, PDF/CSV/Excel 리포트를 자동화하는 오픈소스 BI 플랫폼입니다. Vanna에서 생성된 SQL을 Shaper 대시보드로 승격(Dashboard Promotion)하여 반복 조회 KPI를 정적 대시보드로 전환합니다.

**최신 버전 주요 기능:**
- **SQL-First BI:** 모든 분석 구성을 SQL 코드로 정의, AI-Ready 구조
- **Git 기반 워크플로:** 대시보드 구성을 코드로 관리하여 버전 관리 및 협업 지원
- **임베디드 분석:** React SDK를 통한 iframe 없는 임베딩, 화이트라벨링, JWT 기반 Row-level Security
- **자동화 보고:** PDF, PNG, CSV, Excel 형식의 보고서 자동 생성, 예약 알림 및 비밀번호 보호 링크 공유
- **DuckDB 엔진:** 경량 고속 분석 엔진으로 별도 DB 인프라 불필요, 다중 데이터 소스 간 쿼리 실행 지원
- **Self-Hosted:** Docker 기반 즉시 배포, MPL-2.0 라이선스

**DataNexus 연동 포인트:**
- Vanna NL2SQL → Shaper Dashboard Promotion (자주 사용되는 SQL을 정적 대시보드로 승격)
- JWT 토큰 공유를 통한 Row-level Security 일관성 유지
- React SDK로 DataNexus Frontend에 iframe 없이 임베딩
- 예약 보고서 자동 생성/발송으로 Push 기반 인사이트 전달

#### alive-analysis v1.1 – 구조화된 데이터 분석 워크플로우 킷
AI 코딩 에이전트(Claude Code, Cursor)에 SKILL.md 기반의 구조화된 분석 사고 프레임워크를 주입하는 오픈소스 워크플로우 킷입니다. 소프트웨어 라이브러리가 아닌 마크다운 기반 방법론으로, 전통적 의존성 없이 동작합니다.

**핵심 설계 원칙:** AI 에이전트는 분석 결론을 생성하지 않으며, 구조화된 질문을 하고 체크리스트를 강제합니다. 모든 분석적 판단은 사람이 내립니다.

**ALIVE 루프 5단계:**
- **Ask:** 질문 정의 — 범위, 가설 트리, 성공 기준
- **Look:** 데이터 관찰 — 품질 점검, 세분화, 교란변수
- **Investigate:** 분석 — 가설 검증, 다중 렌즈 프레임워크
- **Voice:** 커뮤니케이션 — "So what → Now what", 청중별 메시징
- **Evolve:** 발전 — 후속 질문, 임팩트 추적, 회고

**5개 모듈 (20개 슬래시 커맨드):**
- 분석(9): ALIVE 루프 Full/Quick 모드, Investigation/Modeling/Simulation 3개 분석 유형
- 실험(3): A/B 테스트 생명주기, SRM 검사, 가드레일 메트릭
- 모니터링(3): North Star→Leading→Guardrail→Diagnostic 4단계 메트릭 분류
- 모델링(1): ML 모델 버전 등록, 드리프트 모니터링
- 교육(4): 7개 연습 시나리오, 루브릭 기반 100점 채점

**DataNexus 연동 포인트:**
- Vanna NL2SQL 쿼리 결과를 ALIVE 루프로 구조화 분석 (Pull → Analyze 워크플로우)
- 실험 모듈로 Ablation Study(PRD_05 §5.4) 품질 표준화 (사전등록, SRM 검사)
- 4단계 메트릭 분류로 DataNexus 평가 지표를 사용자 관점에서 계층화 (PRD_05 §5.1.4)
- Git 추적 분석 파일로 조직의 분석 지식베이스 자동 축적 (방어선 전략 기여)
...
| **Dashboard & Report** | **BI & Reporting Engine** | **Shaper (DuckDB)** | **SQL 기반 대시보드, PDF/CSV/Excel 자동 보고, 임베디드 분석** |
...
#### 2.1.3.1 Dashboard Promotion (Shaper 연동) 플로우
```txt
[Vanna NL2SQL 결과] → [사용자: "대시보드로 저장"]
    │
    ▼
┌────────────────────────────────────────────────────────────────┐
│                  Dashboard Promotion Engine                      │
│   ① SQL 추출 (Vanna 생성 SQL + 파라미터)                       │
│   ② Shaper Dashboard 등록 (SQL-as-Code)                        │
│   ③ JWT Row-level Security 연동                                │
│   ④ 스케줄 설정 (일/주/월 자동 리포트)                          │
└────────────────────────┬───────────────────────────────────────┘
                         │
           ┌─────────────┼─────────────┐
           │             │             │
    ┌──────▼──────┐ ┌────▼────┐ ┌──────▼──────┐
    │ Static      │ │ Report  │ │ Embedded    │
    │ Dashboard   │ │ Auto    │ │ Analytics   │
    │ (KPI 모니터)│ │ (PDF/   │ │ (React SDK, │
    │             │ │  Excel) │ │  iframe-free)│
    └─────────────┘ └─────────┘ └─────────────┘
```

**Dashboard Promotion 핵심 원칙:**
- **Pull → Push 전환:** 자주 사용되는 NL2SQL 질의를 정적 대시보드로 승격하여, 사용자가 매번 질문하지 않아도 KPI를 모니터링
- **SQL-as-Code:** Shaper의 Git 기반 워크플로를 활용하여 대시보드 구성을 코드로 관리, 버전 관리 및 코드 리뷰 가능
- **보안 일관성:** DataNexus의 JWT 토큰을 Shaper와 공유하여 Row-level Security가 대시보드에서도 동일하게 적용
...
| **Vanna ↔ Shaper** | **REST API / SQL Promotion** | **NL2SQL 결과를 대시보드/리포트로 승격** |
| **Shaper ↔ Data Sources** | **DuckDB + 커넥터** | **PostgreSQL, CSV, Parquet 등 다중 소스 쿼리** |
| **Shaper ↔ Frontend** | **React SDK (iframe-free)** | **임베디드 대시보드, JWT Row-level Security** |
...
| **Shaper** | **`http://localhost:5454`** | **SQL 기반 대시보드 & 자동화 보고** |

---

### 2.2 에이전트 프레임워크 용어 정의
...
### 2.3 DozerDB Multi-Database 격리 구조 명확화
