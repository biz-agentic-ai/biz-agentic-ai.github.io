...
말로 하면 와닿지 않는다. 5개 인터랙티브 데모를 만들어 뒀다. 각각 독립적이니 관심 가는 것부터 열어보면 된다.

### 같은 질문, 다른 출처

"가족 여행 호텔 추천해줘." 이 질문을 Genspark, Perplexity, ChatGPT에 동시에 던져봤다. 세 AI 모두 비슷한 답을 내놓는다. 수영장 정보, 객실 가격, 조식까지. 답변 품질은 이미 충분하다.

AI 답변 자체는 이미 충분하다. 문제는 출처다. Genspark은 공식 사이트의 Schema 데이터를 직접 인용하고, Perplexity는 네이버 블로그와 여기어때를 긁어온다. ChatGPT는 공식 사이트를 참조하지만 구조화 데이터 없이는 정밀도가 떨어진다. 같은 호텔인데 AI마다 보여주는 가격이 다르다.
...
### robots.txt - AI 크롤러에 문을 열었는가

GEO 이전에 확인할 게 있다. robots.txt다. AI 크롤러(GPTBot, ClaudeBot, PerplexityBot)가 사이트에 접근할 수 있는지를 결정하는 파일인데, 같은 그룹사끼리도 정책이 제각각인 경우가 많다. 어떤 계열사는 전면 허용, 어떤 곳은 전면 차단. 전면 차단한 사이트는 AI가 공식 정보를 못 가져가니 블로그나 커뮤니티를 인용하게 된다.

<div class="demo-btn-group">
<a href="/demo/geo-demo-b.html" target="_blank" rel="noopener" onclick="window.open(this.href,'geo-b','width=1200,height=800,scrollbars=yes,resizable=yes');return false;" class="demo-btn demo-btn--outline"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>Demo - robots.txt 현황</a>
</div>

### 상품 데이터 Before/After

JSON-LD 없이 검색하면 AI가 가격도, 재고도, GTIN도 모른다. 답변이 뭉뚱그려지거나 아예 엉뚱한 정보를 만들어낸다. FatSecret이나 이마트몰 같은 제3자 사이트가 출처로 잡히고, 가격도 채널마다 다르다.

`{"@type": "Product", "gtin13": "..."}` 한 줄을 넣으면 상황이 바뀐다. AI가 공식 사이트를 출처로 걸고, 가격과 스펙을 정확하게 뽑아낸다. 브랜드가 AI 답변의 출처를 통제할 수 있게 되는 거다.

<div class="demo-btn-group">
<a href="/demo/geo-demo-c1.html" target="_blank" rel="noopener" onclick="window.open(this.href,'geo-c1','width=1200,height=800,scrollbars=yes,resizable=yes');return false;" class="demo-btn demo-btn--outline"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>Demo - 상품 Before/After</a>
</div>

### 호텔 데이터 Before/After

호텔은 상품보다 복잡하다. 객실 타입, 수영장 운영시간, 조식 가격, 주변 관광지까지 구조화할 데이터가 많다. Hotel Schema와 FAQ Schema를 함께 적용하면 AI가 "수영장 있어?" 같은 질문에 공식 데이터로 답할 수 있게 된다. OTA 가격 변동에 흔들리지 않는 브랜드 통제 정보다.

<div class="demo-btn-group">
<a href="/demo/geo-demo-c2.html" target="_blank" rel="noopener" onclick="window.open(this.href,'geo-c2','width=1200,height=800,scrollbars=yes,resizable=yes');return false;" class="demo-btn demo-btn--outline"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>Demo - 호텔 Before/After</a>
</div>

### JSON-LD 빌더

코드를 몰라도 된다. 폼에 상품명, 가격, GTIN을 입력하면 JSON-LD가 자동 생성된다. Product, Hotel, Article, FAQ 네 가지 스키마를 지원한다. 검증 버튼으로 문법 체크하고, 복사해서 HTML `<head>` 에 붙여넣으면 끝이다.

<div class="demo-btn-group">
<a href="/demo/geo-demo-d.html" target="_blank" rel="noopener" onclick="window.open(this.href,'geo-d','width=1200,height=800,scrollbars=yes,resizable=yes');return false;" class="demo-btn demo-btn--outline"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>Demo - JSON-LD 빌더</a>
</div>
