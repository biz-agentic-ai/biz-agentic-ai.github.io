...
SEO를 열심히 해서 구글 검색 1페이지에 올라갔다. 오가닉 트래픽이 늘었다. 여기까지는 익숙한 시나리오다.
...
GS1 GTIN/GLN 같은 국제 표준 식별자가 핵심이다. "빼빼로 초코"와 "빼빼로 아몬드"를 AI가 별개 상품으로 인식하려면 각각 고유한 GTIN이 있어야 한다. 대표코드 하나로 묶어놓으면 AI는 둘을 구분하지 못한다.
...
<script type="application/ld+json">
{
  ...
  "@type": "Product",
  "name": "빼빼로 초코",
  "gtin13": "8801056038800",
  "brand": { "@type": "Brand", "name": "롯데웰푸드" },
  "offers": {
    "@type": "Offer",
    "price": 1500,
    "priceCurrency": "KRW",
...
말로 하면 와닿지 않는다. AI 검색 결과가 실제로 어떻게 달라지는지 시뮬레이션할 수 있는 데모를 만들어 뒀다.

[**GEO 인터랙티브 데모 열기**](/demo/geo-demo.html)

봐야 할 포인트는 두 가지다.

JSON-LD 없이 검색하면 AI가 가격도, 재고도, GTIN도 모른다. 답변이 뭉뚱그려지거나 아예 엉뚱한 정보를 만들어낸다(Hallucination). `{"@type": "Product", "gtin13": "..."}` 같은 구조화 데이터를 넣으면 상황이 확 바뀐다. AI가 가격, 스펙, 구매 링크까지 정확하게 뽑아내고, 출처로 우리 사이트를 건다.

JSON-LD 한 줄이 만드는 차이가 이 정도다.
