...
Type 1이 맞는 경우가 있다. 오타 수정이 대표적이다. "서울특별시"를 "서울시"로 바꾸는 건 이력을 남길 이유가 없다. 코드 테이블의 설명 문구 변경, 고객 이름의 오타 교정. 과거를 알 필요가 없는 속성에 쓴다.
...
여기서 `dim_customers_sk` 가 서로게이트 키(surrogate key)다. 한 고객에 여러 행이 생기니까 `customer_id`만으로는 행을 고유하게 식별할 수 없다. 별도 대리 키가 필요한 이유다. 설계 세부 내용은 Gold 편에서 다룬다.
...
쓸 만한 사례가 있다. 조직 개편 전후 비교. "이 사원이 이번 개편 전에는 어느 부서였는가"만 알면 되는 경우. 직전 값 하나면 충분하고, 전체 이력은 필요 없다.
...
판단 기준은 간단하다. "과거 시점의 값으로 분석해야 하는가?" 그렇다면 Type 2. 아니라면 Type 1. Type 3은 직전 값만 필요한 특수한 경우에 한정된다.
...
위에서 Type 2를 SQL로 직접 구현했다. 기존 행을 닫고, 새 행을 넣고, `valid_from`/`valid_to`를 관리하고. 테이블이 하나일 때는 할 만하다. 차원 테이블이 10개, 20개로 늘어나면 이 로직을 매번 직접 짜는 건 현실적이지 않다.
...
실무에서는 Type 2가 압도적이다. dbt snapshot이 처리해주니까 구현 부담도 크지 않다. Type 1은 이력이 필요 없는 속성에 한해서, Type 3은 직전 값만 필요한 드문 경우에 쓴다.
...
핵심은 `run_snapshot >> run_staging` 순서다. Silver 모델 중 `stg_customers_hist`가 `snap_customers`를 참조한다. snapshot이 먼저 실행되어야 Silver가 최신 이력을 반영한다. snapshot을 Silver 뒤에 돌리면 이번 배치에서 감지된 변경이 다음 배치에서야 Silver에 반영된다. 하루 늦는다.
